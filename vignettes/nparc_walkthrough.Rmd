---
title: "Analysing thermal proteome profiling data with the NPARC pacakge"
...
# Introduction
This vignette shows how to reproduce the analysis described by [Childs, Bach, Franken et al. (2019): Non-parametric analysis of thermal proteome profiles reveals novel drug-binding proteins](https://www.biorxiv.org/content/10.1101/373845v2) using the `NPARC` package.
...
First, we load data from a staurosporine TPP experiment [@Savitski2014]. The necessary ETL (extract, transform, load) steps have already been conducted to download the data from the supplements of the respective publication and convert them into tidy format (separate vignette to follow). 
...
- `uniqueID`: The unique identifier for each protein. In the given dataset, it contains the gene symbol concatenated by IPI id: `r head(df$uniqueID)` .
...
The imported data contains contains **`r nrow(df)`** rows with entries for **`r length(unique(df$uniqueID))`** proteins.
...
This section describes the different steps of the NPARC workflow for model fitting and hyothesis testing. Note that the package also provides a function `nparc()` that performs all of the following steps with one single function call.
...
The results of the STK4 example from earlier can be found in this object as follows.
...
fStats <- nparcFtest(modelMetrics, df_type = "theoretical")
```
...
ggplot(filter(fStats, !is.na(pAdj))) +
  geom_density(aes(x = F), fill = "steelblue", alpha = 0.5) +
  geom_line(aes(x = F, y = df(F, df1 = df1, df2 = df2)), color = "darkred", size = 1.5) +
  theme_bw() +
...
```{r testDOFemp}
modelMetrics <- fits$metrics 
fStats <- nparcFtest(modelMetrics, df_type = "empirical")
```
